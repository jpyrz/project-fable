...
export function Expeditions() {
  ...
  return <div className={styles.page}>
    ...
    <header className={`${styles.hero} ${styles[location.replaceAll('-', '')]}`}><div><span>COMPANION EXPEDITIONS</span><h1>{selectedLocation.name}</h1><p>{selectedLocation.description}</p><div><b><MapPin /> {selectedLocation.flavor.split(' · ')[0]}</b><b><Sparkles /> Rare discoveries</b></div></div><div className={styles.heroPet}><PetAvatar pet={activePet} showBackground={false} /><span>{activePet.name} is ready!</span></div></header>
