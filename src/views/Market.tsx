...
function SellPanel({ state, initialItemId, onCreate }: { state: ReturnType<typeof useGame>['state']; initialItemId:string; onCreate:(itemId:string,quantity:number,price:number)=>Promise<boolean> }) {
  const sellable=items.filter((item)=>(state.inventory[item.id]??0)>0)
  const [itemId,setItemId]=useState((state.inventory[initialItemId]??0)>0?initialItemId:sellable[0]?.id??''); const [quantity,setQuantity]=useState(1); const [price,setPrice]=useState(50)
