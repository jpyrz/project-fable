...
function SellPanel({ state, onCreate }: { state: ReturnType<typeof useGame>['state']; onCreate:(itemId:string,quantity:number,price:number)=>Promise<boolean> }) {
  ...
  return <section className={styles.sellPanel}><div className={styles.sellArt}>{item?.icon??'📦'}</div><div><span>OPEN A MARKET STALL</span><h2>List something from your bag</h2><p>A 5% market fee is taken when it sells. Listed items stay safely in escrow.</p><label>Treasure<select value={itemId} onChange={(event)=>setItemId(event.target.value)}>{sellable.map((entry)=><option key={entry.id} value={entry.id}>{entry.name} (×{state.inventory[entry.id]})</option>)}</select></label><div className={styles.sellFields}><label>Quantity<input type="number" min="1" max={state.inventory[itemId]??1} value={quantity} onChange={(event)=>setQuantity(Number(event.target.value))}/></label><label>Price each<input type="number" min="1" value={price} onChange={(event)=>setPrice(Number(event.target.value))}/></label></div><button disabled={!itemId||quantity<1||quantity>(state.inventory[itemId]??0)||price<1} onClick={()=>void onCreate(itemId,quantity,price)}>List for {quantity*price} ✦</button></div></section>
}
