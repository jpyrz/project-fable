...
export function Profile() {
  ...
  return <div className={styles.page}><section className={styles.banner}><div className={styles.identity}><div className={styles.avatar}>{state.username.slice(0, 1)}</div><div><span>KEEPER PROFILE</span><h1>{state.username}</h1><p>Making Bramblewick brighter, one tiny treasure at a time.</p></div></div><div className={styles.pet}><PetAvatar pet={activePet} /><div><span>ACTIVE FABLE</span><h2>{activePet.name}</h2><p>{getSpecies(activePet.speciesId).name}</p></div></div></section>
    <section className={styles.stats}><article><Star /><b>Level {state.reputation}</b><span>{state.reputationXp}/100 reputation</span><div><i style={{ width: `${state.reputationXp}%` }} /></div></article><article><BookOpen /><b>{state.collected.length}/120</b><span>Collection discovered</span></article><article><Users /><b>{state.friends.length} friends</b><span>{state.friends.filter((friend) => friend.online).length} exploring now</span></article></section>
    <section className={styles.friendFinder}><UserPlus /><div><span>ADD A FRIEND</span><h2>Know another Keeper?</h2><p>Enter their exact Keeper name. DMs unlock after they accept.</p></div><label><input value={friendName} onChange={(event) => setFriendName(event.target.value)} placeholder="Keeper name" /><button disabled={friendName.length<3} onClick={() => void addFriend()}>Send request</button></label>{friendStatus && <small role="status">{friendStatus}</small>}</section>
    ...
    <section className={styles.showcase}><header><div><span>SHOWCASE</span><h2>Favorite finds</h2></div><Award /></header><div>{state.collected.slice(-6).map((id) => { const item = items.find((entry) => entry.id === id)!; return <article key={id}><b>{item.icon}</b><span>{item.name}</span></article> })}</div></section>
    <section className={styles.showcase}><header><div><span>WISHLIST</span><h2>Dreaming of these</h2></div><Heart /></header><div>{state.wishlist.map((id) => { const item = items.find((entry) => entry.id === id)!; return <article key={id}><b>{item.icon}</b><span>{item.name}</span></article> })}</div></section>
