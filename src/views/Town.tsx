...
export function Town() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.dailies}><header><div><span>TODAY IN BRAMBLEWICK</span><h2>Little daily adventures</h2><small>New adventures in {resetLabel}</small></div><b>{state.tasks.filter((t) => t.claimed).length}/{state.tasks.length}</b></header>
      <div className={styles.taskGrid}>{state.tasks.map((task) => <article key={task.id}><div><span>{task.kind === 'care' ? '💗' : task.kind === 'play' ? '🎮' : '🎁'}</span><div><strong>{task.label}</strong><small>{task.progress}/{task.target} complete</small></div></div><div className={styles.progress}><i style={{ width: `${task.progress / task.target * 100}%` }} /></div><button disabled={task.claimed || task.progress < task.target} onClick={() => void claimTask(task.id)}>{task.claimed ? 'Claimed' : `Claim ${task.reward} ✦`}</button></article>)}</div>
    </section>
  </div>
}
