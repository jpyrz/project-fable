...
export function Town() {
  ...
  const wish = async () => { await makeWish(); celebrate({ level: 'major', icon: '🌠', title: 'Your wish came true!', detail: 'The well gifted you 75 Dewdrops.' }) }
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.map}>
      ...
      <button aria-label="Make a wish at the Wishing Well" className={`${styles.pin} ${styles.well}`} onClick={() => void wish()} disabled={state.dailyWishClaimed}><Sparkles /><span>{state.dailyWishClaimed ? 'Wish made!' : 'Wishing Well'}</span></button>
    </section>
