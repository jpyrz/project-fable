...
export function Friends() {
  ...
  return <div className={styles.page}>
    ...
    {state.friendRequests.length > 0 && <section className={styles.requests}><header><div><span>WAITING FOR YOU</span><h2>Friend requests</h2></div><b>{state.friendRequests.length}</b></header>{state.friendRequests.map((requester) => <article key={requester.requesterId}><div className={styles.avatar}>{requester.name.slice(0,1)}</div><div><strong>{requester.name}</strong><small>would like to be friends</small></div><button className={styles.accept} onClick={() => void respondFriendRequest(requester.requesterId, true)} aria-label={`Accept ${requester.name}`}><Check /></button><button className={styles.decline} onClick={() => void respondFriendRequest(requester.requesterId, false)} aria-label={`Decline ${requester.name}`}><X /></button></article>)}</section>}
