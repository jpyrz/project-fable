import { Check, Eye, HeartHandshake, MessageCircle, UserPlus, Users, X } from 'lucide-react'
...
export function Friends() {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.list}><header><div><span>KEEPERS YOU KNOW</span><h2>Your friends</h2></div><Users /></header>{state.friends.length ? <div className={styles.grid}>{state.friends.map((friend) => <article key={friend.id}><div className={styles.avatar}>{friend.name.slice(0,1)}<i className={friend.online ? styles.online : ''} /></div><div><strong>{friend.name}</strong><small>Exploring with {friend.pet}</small><span>{friend.online ? 'Around Bramblewick' : 'Away for now'}</span></div><div className={styles.friendActions}><Link to={`/keeper/${encodeURIComponent(friend.name)}`}><Eye /> Profile</Link><Link to={`/plaza?dm=${encodeURIComponent(friend.name)}`}><MessageCircle /> Message</Link></div></article>)}</div> : <div className={styles.empty}><HeartHandshake /><h3>Your circle is ready to grow</h3><p>Send a request above, or say hello in the Lobby.</p><Link className="button secondary" to="/plaza">Visit the plaza</Link></div>}</section>
  </div>
}
